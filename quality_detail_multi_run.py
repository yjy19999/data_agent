#!/usr/bin/env python3
"""
Run the DataQualityDetailMultiRunner against one or more data files.

Like quality_detail_run.py, this guarantees full block coverage of every
data record.  The key difference: each JSONL record is inspected by a
**fresh sub-agent** with a clean context window — eliminating cross-record
context accumulation ("memory leak") when processing large datasets.
...
from agent.data_quality_detail_multirun import DataQualityDetailMultiRunner
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the multi-agent detailed data-quality inspection workflow "
            "(fresh sub-agent per JSONL record, full block coverage)"
        )
    )
    parser.add_argument(
        "inputs", nargs="*",
    ...
    parser.add_argument(
        "--consolidation-interval", type=int, default=50, metavar="N",
        help="Main agent writes ObservationSummary.json every N records (default: 50)",
    )
    ...
    cfg_table.add_row("Mode",         "[bold magenta]Multi-agent (fresh agent per JSONL record)[/bold magenta]")
    cfg_table.add_row("Parallelism",  parallel_label)
    cfg_table.add_row("Consolidate",  f"every {args.consolidation_interval} records")
    ...
    runner = DataQualityDetailMultiRunner(
        workspace=workspace,
        config=config,

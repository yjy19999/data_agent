...
python task_run.py --model Codex-opus-4-6 "Build Y"
...
**OpenCode Agent** is a multi-provider AI agent CLI framework that provides a unified interface for running agents with different LLM backends (Codex, Gemini, GPT, Qwen, local models via Ollama) through any OpenAI-compatible API.
...
└── tools/
    ...
    ├── Codex.py     → Codex style tools (Bash, Read, Edit, Write, LS, etc.)
...
- `Codex-*` → `Codex` profile (Codex-style: `Bash`, `Read`, `Edit`, etc.)
...
| `LLM_TOOL_PROFILE` | `auto` | `auto`, `Codex`, `gemini`, `gpt`, `qwen`, `default`, `readonly`, `minimal` |
